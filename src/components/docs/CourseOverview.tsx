...
export default function CourseOverview({ 
  ...
  const formattedPrice = course.price != null && course.price > 0
  ...
  return (
    <div className="space-y-8">
      ...
      {!course.isEnrolled && (
        <div className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6 shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-slate-800 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-5">
          <div className="space-y-1.5 max-w-xl">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-accent/10 px-3 py-1 text-xs font-semibold text-accent">
              ...
              <span>Chưa đăng ký khóa học</span>
            </div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">
              Đăng ký để mở khóa toàn bộ bài học
            </h3>
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
              Quyền truy cập không giới hạn {course.chapters?.length || 0} chương với {totalLessons} bài học chuyên sâu.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto shrink-0">
            ...
            {isFree ? (
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-accent px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-accent/90 transition cursor-pointer w-full sm:w-auto"
              >
              ...
              <div className="flex items-center justify-between sm:justify-start gap-3 bg-gray-50 dark:bg-slate-900/60 border border-gray-200 dark:border-slate-700 rounded-xl p-1.5 pl-3.5">
                <div>
                  <div className="text-[11px] font-medium text-gray-500 dark:text-gray-400 leading-none">
                    Mua lẻ khóa học
                  </div>
                  <div className="text-base sm:text-lg font-bold text-accent tabular-nums leading-tight mt-1">
                    {formattedPrice}
                  </div>
                </div>
                <button
                  onClick={() => onEnrollClick?.()}
                  className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-accent px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-xs hover:bg-accent/90 transition cursor-pointer shrink-0"
                >
            ...
            {/* Nút đăng ký gói Premium */}
            {!isFree && (
              <Link
                href="/pricing"
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-amber-200/90 dark:border-amber-900/50 bg-amber-50/70 dark:bg-amber-950/30 hover:bg-amber-100/80 dark:hover:bg-amber-900/50 px-4 py-3 text-xs sm:text-sm font-semibold text-amber-800 dark:text-amber-300 transition shadow-xs text-center shrink-0"
              >
