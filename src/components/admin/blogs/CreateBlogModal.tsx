...
export default function CreateBlogModal({ isOpen, onClose, onSuccess }: CreateBlogModalProps) {
    ...
    const handleInputChange = (field: keyof CreateBlogRequest, value: string | BlogType) => {
    ...
    const handleImageUpload = (file: File) => {
        ...
        if (!file.type.startsWith('image/')) {
            setErrors(prev => ({ ...prev, featuredImage: 'Vui lòng chọn file ảnh hợp lệ' }));
            return;
        }

        // Validate file size (max 5MB)
        if (file.size > 5 * 1024 * 1024) {
            setErrors(prev => ({ ...prev, featuredImage: 'Kích thước ảnh không được vượt quá 5MB' }));
            return;
        }
                ...
                <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl">
                    ...
                    <div className="flex items-center justify-between p-6 border-b border-gray-200">
                        <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">Chia sẻ kiến thức và kinh nghiệm của bạn</p>
                        </div>
                        <button
                            onClick={handleClose}
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => handleInputChange('blogType', e.target.value as BlogType)}
                                    ...
                                    >
                                        {Object.entries(BlogTypeDisplayNames).map(([key, displayName]) => (
                            ...
                            >
                                {isLoading ? (
                                    <>
                                        ...
                                        {isUploadingImage ? 'Đang tải ảnh...' : 'Đang tạo bài viết...'}
                                    </>
                                ) : (
                                    <>
