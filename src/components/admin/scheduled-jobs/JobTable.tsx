...
interface JobTableProps {
  ...
  onSelectJob?: (job: ScheduledJobResponse) => void;
}
...
export default function JobTable({
  ...
  const [internalSelectedJob, setInternalSelectedJob] =
    useState<ScheduledJobResponse | null>(null);

  const selectedJob = internalSelectedJob;
  const handleSelectJob = (job: ScheduledJobResponse) => {
    if (onSelectJob) {
      onSelectJob(job);
    } else {
  ...
  if (isLoading && jobs.length === 0) {
    return (
      <div className="overflow-hidden rounded-2xl border border-border bg-card p-12 text-center text-sm text-muted-foreground shadow-sm">
  ...
  if (jobs.length === 0) {
    return (
      <div className="overflow-hidden rounded-2xl border border-border bg-card p-12 text-center shadow-sm">
        <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-muted">
          <ListChecks className="h-5 w-5 text-muted-foreground" />
        </div>
        <p className="text-sm font-medium text-foreground">Không có job nào</p>
        ...
        <div className="flex items-center justify-between px-1 md:hidden">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Nhấn vào job để xem đầy đủ chi tiết
            </p>
          </div>
          {totalElements > 0 && (
          ...
          <div className="flex items-center justify-between border-b border-border px-5 py-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                Sắp xếp từ job được tạo mới nhất (nhấn để xem chi tiết)
              </p>
            </div>
            {totalElements > 0 && (
              <span className="whitespace-nowrap rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-600 dark:bg-blue-900/30 dark:text-blue-400">
