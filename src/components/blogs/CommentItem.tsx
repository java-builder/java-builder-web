...
export default function CommentItem({
  ...
  console.log(
    "CommentItem rendered for comment:",
    comment.id,
    "onDelete exists:",
    !!onDelete,
  );
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            console.error(
              "Error loading replies after creating new reply:",
              err,
            );
          }
        }
          ...
          <div className="flex items-center space-x-3 mt-3">
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-1.5 text-xs font-medium rounded-md border transition-all duration-200 ${
                showReplyForm
                  ? "bg-accent-50 text-accent-600 border-accent-300"
            ...
            {repliesCount > 0 && (
              <button
                ...
                className="flex items-center space-x-1.5 px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-accent-50 hover:text-accent-600 border border-gray-300 hover:border-accent-300 rounded-md transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoadingReplies ? (
                  <>
                    <svg
            ...
            <div className="mt-3 ml-4 space-y-3 border-l border-gray-200 pl-4">
              {comment.replies.map((reply) => (
                <div
                  key={reply.id}
                  id={`comment-${reply.id}`}
                  className="flex items-start space-x-2 scroll-mt-24"
                >
          ...
          {showReplyForm && (
            <div className="mt-3 bg-gray-50 rounded-lg p-3 border border-gray-200">
              <div className="flex items-start space-x-2">
                <div className="w-7 h-7 bg-accent rounded-full flex items-center justify-center text-white text-xs flex-shrink-0">
                  <svg
                    className="w-3 h-3"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                ...
                <div className="flex-1 min-w-0">
                  <div className="relative">
                    <textarea
                      ...
                      className={`w-full p-2 border rounded-lg resize-none focus:ring-1 focus:ring-accent focus:border-accent text-sm placeholder-gray-400 ${
