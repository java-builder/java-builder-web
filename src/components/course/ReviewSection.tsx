    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            {[5, 4, 3, 2, 1].map(star => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 rounded-lg px-2 py-1 -mx-2 transition-all ${isSelected ? "bg-amber-50" : "hover:bg-gray-50"}`}
                >
                  ...
                  <span className={`text-sm w-8 text-right ${isSelected ? "text-amber-600 font-medium" : "text-gray-500"}`}>{count}</span>
                </button>
              );
            })}
          </div>
        ...
        {filter !== "all" && (
          <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
            <span className="text-sm text-gray-600">Đang lọc: <span className="font-medium">{filter} sao</span> ({filteredReviews.length})</span>
            <button onClick={() => setFilter("all")} className="text-sm text-accent hover:text-accent-600 font-medium">Xóa bộ lọc</button>
          </div>
        )}
      </div>
      ...
      {canReview && (
        <div className="bg-gray-50 rounded-xl p-4">
          <div className="flex items-start gap-3">
            {currentUser?.avatar ? (
              <Image src={currentUser.avatar} alt="" width={40} height={40} className="w-10 h-10 rounded-full object-cover" />
            ) : (
            ...
            <div className="flex-1">
              ...
              <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Chia sẻ trải nghiệm của bạn..." className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent bg-white" rows={3} />
              <div className="flex justify-end mt-3">
                <button onClick={handleSubmit} disabled={!rating || isSubmitting} className="px-5 py-2 bg-accent hover:bg-accent-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2">
      ...
      {filteredReviews.length > 0 ? (
        <div className="space-y-4">
          {filteredReviews.map(review => (
            <div key={review.id} className="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow">
              <div className="flex items-start gap-3">
                {review.userAvatar ? (
                  <Image src={review.userAvatar} alt="" width={40} height={40} className="w-10 h-10 rounded-full object-cover" />
                ) : (
        ...
        <p className="text-center py-8 text-sm text-gray-500">Không có đánh giá {filter} sao</p>
      ) : (
