      ...
      <div className="text-center mb-5">
        {isEnrolled ? (
          ...
          <>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {isFree ? t("courseDetail.freeCourseLifetime") : t("courseDetail.lifetimeAccess")}
            </p>
          </>
        )}
      </div>
      ...
      <div className="space-y-2.5 mb-6">
        {isEnrolled ? (
          ...
          <button
            ...
            className="w-full bg-accent hover:bg-accent/90 text-white font-bold py-2.5 px-4 rounded-lg transition-all duration-200 shadow-xs hover:shadow-md active:scale-98 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isEnrollingFree && (
            ...
            <span>{t("courseDetail.joinNow")}</span>
          </button>
        ) : (
          <button
            onClick={onPayment}
                  ...
                  <svg
                    ...
                    className={`w-3.5 h-3.5 fill-current ${
                      star <= Math.round(averageRating)
                        ? "text-amber-400"
                        : "text-gray-250 dark:text-slate-600"
                    }`}
                    viewBox="0 0 20 20"
                  >
