      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-6 py-8">
          <div className="text-center">
            ...
            {answer && (
              <div className="bg-accent-50 border border-accent-200 rounded-lg p-4 text-left max-w-4xl mx-auto mb-6">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-left max-w-4xl mx-auto mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-green-800 font-medium">
                    Lộ trình học tập đã được lưu thành công!
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <button
                ...
                className="px-8 py-3 bg-accent hover:bg-accent-600 text-white font-medium rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg hover:shadow-xl cursor-pointer"
              >
                {isSaving ? (
                  <>
                    <svg
        ...
        <div className="space-y-6">
          {learningPath.periods && learningPath.periods.length > 0 ? (
            learningPath.periods.map((period) => (
              ...
              >
                {/* Period Header */}
                <div className="bg-gradient-to-r from-accent-50 to-amber-50 px-6 py-4 border-b border-accent-200">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-accent rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg">
                      {period.periodNumber}
                    </div>
                    <div>
                  ...
                  {period.topics && period.topics.length > 0 ? (
                    <div className="space-y-4">
                      {period.topics.map((topic, topicIndex) => (
                        ...
                        >
                          {/* Topic Header */}
                          <div className="bg-gradient-to-r from-accent-50 to-amber-50 px-4 py-3 border-b border-accent-200">
                            ...
                            <div className="flex items-center space-x-4 mt-2 text-xs text-gray-600">
                              {topic.lessons && topic.lessons.length > 0 && (
                                <span className="flex items-center space-x-1 bg-accent-100 text-accent-700 px-2 py-1 rounded-full">
                                  <span>📖</span>
                          ...
                          {topic.lessons && topic.lessons.length > 0 && (
                            <div className="p-4 bg-accent-50/30">
                              <h4 className="text-sm font-semibold text-accent-800 mb-3 flex items-center gap-2">
                              ...
                              <div className="space-y-3">
                                {topic.lessons.map((lesson, lessonIndex) => (
                                  <div
                                    key={`${period.periodNumber}-${topicIndex}-lesson-${lessonIndex}`}
                                    className="bg-white border-l-4 border-accent-400 rounded-r p-3 shadow-sm"
                                  >
                                    ...
                                    {lesson.keywords &&
                                      lesson.keywords.length > 0 && (
                                        <div>
                                          <h6 className="font-medium text-accent-700 text-sm mb-2">
                                            Khái niệm chính:
                                          </h6>
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                            {lesson.keywords.map(
                                              (keyword, keywordIndex) => (
                                                <div
                                                  key={keywordIndex}
                                                  className="p-2 bg-accent-50 border border-accent-200 rounded"
                                                >
                                                  <div className="font-medium text-accent-900 text-sm">
                                                    {keyword.term}
                                                  </div>
                                                  <div className="text-accent-700 text-xs mt-1">
                                                    {keyword.explanation}
                                                  </div>
                                                </div>
                                              ),
                          ...
                          {topic.exercises && topic.exercises.length > 0 && (
                            <div className="p-4 bg-accent-50/30 border-t border-accent-200">
                              <h4 className="text-sm font-semibold text-accent-800 mb-3 flex items-center gap-2">
                                <span className="text-accent-600">💪</span>
                              ...
                              <div className="space-y-3">
                                {topic.exercises.map(
                                  (exercise, exerciseIndex) => (
                                    <div
                                      key={`${period.periodNumber}-${topicIndex}-exercise-${exerciseIndex}`}
                                      className="bg-white border-l-4 border-accent-400 rounded-r p-3 shadow-sm"
                                    >
                                            ...
                                            <div className="space-y-2">
                                              {exercise.instructions.map(
                                                (step, stepIndex) => (
                                                  ...
                                                  >
                                                    <div className="w-5 h-5 bg-accent text-white rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
                                                      {step.stepNumber}
                                                    </div>
                                                    <div className="flex-1">
                                                      ...
                                                      {step.command && (
                                                        <div className="mt-1 p-2 bg-accent-50 border border-accent-200 rounded font-mono text-xs">
                                                          {step.command}
                                                        </div>
                                                      )}
                                                      {step.codeBlock && (
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-8 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                Chưa có giai đoạn học tập
              </h3>
              <p className="text-gray-600">
                Lộ trình học tập chưa có giai đoạn nào được định nghĩa.
              </p>
            </div>
          )}
        </div>
