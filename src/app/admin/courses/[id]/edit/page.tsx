...
import { courseApi, chapterApi, fileApi } from "@/services/course.service";
import { CourseDetailResponse, CourseLevel, ChapterDetailResponse } from "@/types/course";
...
export default function EditCoursePage() {
  ...
  const handleDelete = async () => {
    try {
      ...
      setDeleteModal({ isOpen: false, type: "chapter", id: "", title: "" });
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
                    ...
                    <div key={chapter.id} className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            onClick={() => setDeleteModal({ isOpen: true, type: "chapter", id: chapter.id, title: chapter.chapterName })}
                      ...
                      {expandedChapters.has(chapter.id) && (
                        <div className="border-t border-gray-200 px-4 py-3">
                          <p className="text-sm text-gray-600">{chapter.description || "Chưa có mô tả"}</p>
                        </div>
                      )}
                    </div>
                  ))}
      ...
      <ConfirmModal
        ...
        onClose={() => setDeleteModal({ isOpen: false, type: "chapter", id: "", title: "" })}
        onConfirm={handleDelete}
        title="Xóa chương"
        message={`Bạn có chắc chắn muốn xóa chương <strong>${deleteModal.title}</strong>?`}
        confirmText="Xóa"
        cancelText="Hủy"
        type="danger"
      />
