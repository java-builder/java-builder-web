...
export default function AdminMessagesPage() {
  ...
  return (
    <div className="p-6">
      <div
        className="bg-white rounded-lg shadow border border-gray-100 overflow-hidden flex"
        style={{ height: "calc(100vh - 160px)" }}
      >
        <div className="w-80 border-r border-gray-100 p-4 overflow-auto">
          <div className="text-sm text-gray-500 mb-3">Cuộc trò chuyện</div>
          <div className="space-y-2">
            {conversations.map((c) => (
              <button
                key={c.id}
                onClick={() => setActiveId(c.id)}
                className={`w-full text-left px-3 py-2 rounded ${c.id === activeId ? "bg-accent-50 border border-accent" : "hover:bg-gray-50"} transition-colors flex items-center gap-3`}
              >
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 p-6 overflow-auto space-y-4 bg-gray-50">
          ...
          <div className="p-4 border-t border-gray-100 flex items-center gap-3">
            <input
              ...
              className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-accent"
            />
            <button
              onClick={handleSend}
