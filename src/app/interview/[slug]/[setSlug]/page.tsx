...
export default function InterviewSetPage() {
  ...
  // Load completed từ localStorage
  useEffect(() => {
    try {
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {(["ALL", "EASY", "MEDIUM", "HARD"] as const).map((d) => (
            <button
              ...
              className={`px-2 py-1.5 text-xs font-semibold rounded-md transition-colors whitespace-nowrap ${
                difficultyFilter === d
                  ...
                  >
                    <div className="flex items-start gap-2.5">
                      ...
                      <span className={`flex-shrink-0 text-xs font-mono mt-0.5 ${
                        isActive ? "text-accent font-bold" : "text-gray-400 dark:text-gray-500"
                      }`}>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      ...
      <div
        className={`lg:hidden fixed inset-0 z-40 transition-opacity ${
          isMobileListOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
      >
        <div
        ...
        />
        <div
          className={`absolute right-0 top-0 bottom-0 w-[85%] max-w-sm bg-white dark:bg-slate-800 shadow-xl transform transition-transform ${
