...
export const courseApi = {
    ...
    create: async (data: CreateCourseRequest) => {
        try {
            const response = await apiClient.post<ApiResponse<CreateCourseResponse>>('/api/v1/courses', data);
    ...
    getById: async (id: string) => {
        try {
            const response = await apiClient.get<ApiResponse<CourseDetailResponse>>(`/api/v1/courses/${id}`);
            return response.data;
        } catch (error) {
