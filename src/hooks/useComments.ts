...
export const useComments = (blogId: string) => {
    ...
    // Load root comments
    const loadRootComments = useCallback(async (page: number = 1, append: boolean = false) => {
        try {
            setIsLoading(true);
            ...
            const commentsData: CommentDetailResponse[] = response.result.result.map(comment => ({
                id: comment.id,
    ...
    const loadReplies = useCallback(async (commentId: string) => {
        try {
            const response = await commentApi.getRepliesByParentId(commentId, { page: 1, size: 10 });

            const replies: CommentDetailResponse[] = (response.result?.result || []).map(reply => ({
                id: reply.id,
        ...
        try {
            ...
            setComments(prev => {
                ...
                if (filteredComments.length === prev.length) {
                    ...
                    console.log('Updated comments after reply deletion:', updatedComments);
                    return updatedComments;
                }

                console.log('Updated comments after root deletion:', filteredComments);
