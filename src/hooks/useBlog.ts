...
export const useBlog = () => {
  ...
  const createBlog = async (data: CreateBlogRequest) => {
    ...
    try {
      ...
      const errorMessage =
        (err as Error)?.message || "Có lỗi xảy ra khi tạo bài viết";
  ...
  const getBlogs = async (params?: {
    ...
    try {
      ...
      const errorMessage =
        (err as Error)?.message || "Có lỗi xảy ra khi tải danh sách bài viết";
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const deleteBlog = async (id: string) => {
    ...
    try {
      ...
      const errorMessage =
        (err as Error)?.message || "Có lỗi xảy ra khi xóa bài viết";
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
